...
function showLoader() {
    ...
    if(categoryNavLinksContainer) categoryNavLinksContainer.style.display = 'none'; // Hide links
...
function showEmptyMessage() {
    hideLoader();
     ...
     if(menuEmptyContainer) {
        menuEmptyContainer.textContent = "Meni je trenutno prazan ili nedostupan.";
...
function parseCSV(csv) {
    ...
    if (priceIndex === -1) console.warn("CSV header 'Price' not found. Prices will be empty.");
    if (descIndex === -1) console.warn("CSV header 'Description' not found. Descriptions will be empty.");
    ...
    return rows.map(row => {
        ...
        const item = {};
        item.Category = (categoryIndex !== -1 ? (values[categoryIndex] || 'Ostalo') : 'Ostalo').trim();
...
function displayMenuPager() {
    ...
    categoryNames.forEach((catName, index) => {
        ...
        link.addEventListener('click', (event) => {
            ...
            const clickedIndex = parseInt(event.target.dataset.categoryIndex, 10);
